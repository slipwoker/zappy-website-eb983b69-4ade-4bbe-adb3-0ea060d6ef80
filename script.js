document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const validateField = function (field) {
      ...
      if (value.trim() !== '' && (type === 'email' || field.name === 'email') && !validators.email(value)) {
        showFieldError(field, 'Please enter a valid email address.');
        return false;
      }

      if (value.trim() !== '' && (type === 'tel' || field.name === 'phone' || field.name === 'tel') && !validators.phone(value)) {
        showFieldError(field, 'Please enter a valid phone number.');
        return false;
      }

      if (value.trim() !== '' && minLength && !validators.minLength(value, minLength)) {
        showFieldError(field, 'Must be at least ' + minLength + ' characters long.');
        return false;
      }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'eb983b69-4ade-4bbe-adb3-0ea060d6ef80',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'eb983b69-4ade-4bbe-adb3-0ea060d6ef80';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'eb983b69-4ade-4bbe-adb3-0ea060d6ef80';
